...
console.log("Using Gemini API Key:", process.env.NEXT_PUBLIC_GEMINI_API_KEY ? "SET" : "NOT SET");
...
8. NO markdown formatting, NO code blocks, ONLY raw JSON`
    ...
    console.error("Error generating UML diagram from prompt:", error)
    return {
      success: false,
...
Generate a complete diagram based on what you observe in the image.`
    ...
    console.error("Error generating UML diagram from image:", error)
    return {
      success: false,
...
7. NO markdown formatting, NO code blocks, ONLY raw JSON`
    ...
    console.error("Error modifying UML diagram:", error)
    return {
      success: false,
      message: "Error modifying diagram with AI service",
      error: error instanceof Error ? error.message : "Unknown error"
    }
  }
}
